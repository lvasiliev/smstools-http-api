...
@auth.login_required
def outgoing_view():
    bad_req_except = '400: Bad Request'
    ...
    if type(request_object) is not dict:
        return bad_request(bad_req_except, 'Wrong JSON object')
    for required_field in required_fields:
        if required_field not in request_object:
            return bad_request(bad_req_except, 'Missing required: {0}'.format(required_field))
    if type(request_object['mobiles']) is not list:
        return bad_request(bad_req_except, "mobiles is not array")
    if len(request_object['mobiles']) == 0:
        return bad_request(bad_req_except, 'mobiles array is empty')
    for mobile in request_object['mobiles']:
        if type(mobile) is not unicode:
            return bad_request(bad_req_except, "mobiles is not unicode")
    if type(request_object['text']) is not unicode:
        return bad_request(bad_req_except, "text is not unicode")
